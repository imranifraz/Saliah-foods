...
import { IconUsers } from "../components/icons/AdminIcons.jsx";
...
function UsersListPage() {
  ...
  function load(nextPage = page) {
    ...
    apiFetch(`/api/admin/users?${params}`)
      .then((d) => {
        const responseTotal = d.total ?? d.users.length;
        const responsePage = d.page ?? nextPage;
        if (d.users.length === 0 && responsePage > 1 && responseTotal > 0) {
  ...
  const columns = ["User", "Contact", "Orders", "Joined", "Actions"];
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
          ...
          <div className="flex min-w-0 items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="admin-muted mt-2 max-w-2xl text-[15px] leading-relaxed">
                View and manage customer accounts, contact details, and storefront access.
              </p>
            </div>
          </div>
          <button
          ...
          <div className="flex flex-col items-center px-6 py-14 text-center">
            ...
            <p className="admin-muted mt-2 max-w-sm text-sm leading-relaxed">
              {query
                ...
                : "Create the first customer account or wait for storefront sign-ups."}
            </p>
            {!query ? (
              <button
                type="button"
          ...
          <>
              ...
              {users.map((user) => (
                <DataRow key={user.id}>
                  <DataCell>
                    ...
                    >
                      <ListAvatar name={user.fullName} avatarUrl={user.avatarUrl} />
                      <div className="min-w-0">
                        <p className="truncate font-semibold text-[var(--admin-fg)]">{user.fullName}</p>
                      </div>
                    </Link>
                  </DataCell>
                  <DataCell>
                    <div className="space-y-0.5">
                      ...
                      <p className="admin-muted truncate text-xs">{formatPhoneDisplay(user.phone) || "—"}</p>
                    </div>
                  </DataCell>
                  <DataCell className="whitespace-nowrap text-sm">{user.orderCount}</DataCell>
                  <DataCell className="admin-muted whitespace-nowrap text-sm">
                    {formatJoinedDate(user.createdAt)}
                  </DataCell>
                  <DataCell>
                    <div className="flex items-center justify-end gap-1.5">
                      <TableIconButton to={`/users/${user.id}`} label="View customer">
                        <IconEye />
                      </TableIconButton>
                      <TableIconButton to={`/users/${user.id}`} label="Edit customer">
                        <IconEdit />
                      </TableIconButton>
                      <TableIconButton
                      ...
                      >
                        {deletingId === user.id ? <span className="text-xs">…</span> : <IconTrash />}
                      </TableIconButton>
                    </div>
                  </DataCell>
                </DataRow>
            ...
            <div className="-mx-5 -mb-5 flex flex-col gap-3 border-t border-[var(--admin-border)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="admin-muted text-sm">
                Showing {showingFrom} to {showingTo} of {total} customer{total === 1 ? "" : "s"}
              </p>
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `Remove "${deleteTarget.fullName}" from the system? This deletes their account and related data permanently.`
            : ""
        }
...
function AdminsListPage() {
  ...
  const columns = ["User", "Contact", "Joined", "Updated", "Actions"];
