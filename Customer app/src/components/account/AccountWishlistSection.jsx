...
function StarRow({ rating, stockLabel, inStock }) {
  if (rating == null && !stockLabel) return null;
  ...
  return (
    <div
      className="wishlist-row__rating"
      aria-label={
        rating != null
          ? `Rated ${displayRating.toFixed(1)} out of 5${stockLabel ? `, ${stockLabel}` : ""}`
    ...
    >
      {rating != null ? (
        <>
          ...
          <span className="font-body text-[11px] font-medium text-emerald-900/50">
            {displayRating.toFixed(1)}
          </span>
        </>
      ) : null}
      {rating != null && stockLabel ? (
      ...
      {stockLabel ? (
        <span
          className={`wishlist-row__stock${inStock === false ? " wishlist-row__stock--out" : ""}`}
        >
          {stockLabel}
        </span>
      ) : null}
    ...
    <article className="wishlist-row">
      ...
      <div className="wishlist-row__body">
        <div className="wishlist-row__details">
          ...
          <StarRow rating={item.rating} stockLabel={item.stockLabel} inStock={item.inStock} />
